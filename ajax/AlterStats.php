...
if($_POST["quantity"] != "" && $_POST["id"] != "" && $_POST["locatio"] != "")
{
	$id = $_POST["id"];
	...
	if( $CONNPDO != null)
	{	
		$getdata_PRST = $CONNPDO->prepare("SELECT COUNT(type) AS number FROM storehouse WHERE id = :id");
		...
		{
			$getdata_PRST = $CONNPDO->prepare("UPDATE storehouse SET quantity = :quantity , place = :place WHERE id=:id");
			$getdata_PRST ->bindValue(":id",$id,SQLITE3_INTEGER);
			...
			while($getdata_RSLT = $getdata_PRST->fetch(PDO::FETCH_ASSOC,PDO::FETCH_ORI_NEXT))
			{
				...
				if ($criteria < 30)
				{
					$response .= "<tr><td>$id</td><td>$type</td><td>$brand</td><td>$weight</td><td id='$id' ondblclick=\"DoubleClick1($id)\">$quantity</td><td>$exDate</td><td>$place</td><td><button class='btn btn-danger' onclick='Delete($id)'>Remove</button></td></tr>";
				}
				else
				{
...
elseif($_POST["quantity"] != "" && $_POST["id"] != "" && $_POST["locatio"] == "")
{
	$id = $_POST["id"];
	$quantity = $_POST["quantity"];
	...
	{	
		...
		while($getdata_RSLT = $getdata_PRST->fetch(PDO::FETCH_ASSOC,PDO::FETCH_ORI_NEXT))//edw pera
		{
			$count = $getdata_RSLT["number"];
		}
		...
		if($count != 0 && $quantity > 0)
		{
			...
			while($getdata_RSLT = $getdata_PRST->fetch(PDO::FETCH_ASSOC,PDO::FETCH_ORI_NEXT))
			{
				...
				if ($criteria < 30)
				{
					$response .= "<tr><td>$id</td><td>$type</td><td>$brand</td><td>$weight</td><td id='$id' ondblclick=\"DoubleClick1($id)\">$quantity</td><td style='color:red;'>$exDate</td><td>$place</td><td><button class='btn btn-danger' onclick='Delete($id)'>Remove</button></td></tr>";
				}
				else
				{
					$response .= "<tr><td>$id</td><td>$type</td><td>$brand</td><td>$weight</td><td id='$id' ondblclick=\"DoubleClick1($id)\">$quantity</td><td>$exDate</td><td>$place</td><td><button class='btn btn-danger' onclick='Delete($id)'>Remove</button></td></tr>";
...
elseif($_POST["quantity"] == "" && $_POST["quantity"] == 0 && $_POST["id"] != "" && $_POST["locatio"] != "")
{
	$id = $_POST["id"];
	$location = $_POST["locatio"];
